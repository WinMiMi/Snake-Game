...
public class HistoryEndlessController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		endlessMode = HistoryController.endlessMode;
		System.out.println("In the endless history : user from Game" + user);

		ObservableList<History> allHistory = historyDAO.getAllScoresByUserIdAndMode(user.getId(),
				endlessMode.toString());
		System.out.println(allHistory.size());
		System.out.println("Endless History" + endlessMode);

		for (Iterator<History> iterator = allHistory.iterator(); iterator.hasNext();) {
			history = (History) iterator.next();
		}

		lblHighestTotalScore.setText(String.valueOf(historyDAO.getHighestScore(user.getId(), endlessMode.toString())));
		lblTotalGamePlayed.setText(String.valueOf(historyDAO.getTotalPlayedGame(user.getId(), endlessMode.toString())));
		if (history != null) {
		...
		LocalTime besttime = historyDAO.getBestTime(user.getId(), endlessMode.toString());
